...
Note that the built-in L<C<S<print FILEHANDLE LIST>>|perlfunc/print>,
L<C<S<say FILEHANDLE LIST>>|perlfunc/say>, L<C<S<exec PROGRAM
LIST>>|perlfunc/exec>, and L<C<S<system PROGRAM LIST>>|perlfunc/system>
forms are not affected, even though they look like indirect object
syntax.
...
on) indirect object syntax on older Perls, see the L<indirect> CPAN
module.
